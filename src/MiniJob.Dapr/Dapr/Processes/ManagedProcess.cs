...
public class ManagedProcess : IProcess
{
    ...
    private ILogger<ManagedProcess> _logger;
    private bool _stopping;

    public ManagedProcess()
    {
        CreateSystemProcess = p => new SystemProcess(p);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        lock (_processLock)
        {
            ...
            try
            {
                ...
                _logger?.LogInformation("Process {ProcessName} PID:{ProcessId} started successfully", _systemProcess.Name, _systemProcess.Id);
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "Error starting process {ProcessFilename}", filename);
