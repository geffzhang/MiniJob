namespace MiniJob.Cron.Parsers;
...
/// </remarks>
internal sealed class RangeParser : CronParserBase, ITimeParser
{
    /// <summary>
    /// 构造函数
    /// </summary>
    ...
    public RangeParser(int start, int end, int? steps, CrontabFieldKind kind)
    {
        ...
        {
            throw new CronExpressionException(string.Format("Start = {0} is out of bounds for <{1}> field.", start, Enum.GetName(typeof(CrontabFieldKind), kind)));
        }

        // 验证终止值有效性
        if (end < 0 || end > maximum)
        {
            throw new CronExpressionException(string.Format("End = {0} is out of bounds for <{1}> field.", end, Enum.GetName(typeof(CrontabFieldKind), kind)));
        }

        // 验证步长有效性
        if (steps != null && (steps <= 0 || steps > maximum))
        {
            throw new CronExpressionException(string.Format("Steps = {0} is out of bounds for <{1}> field.", steps, Enum.GetName(typeof(CrontabFieldKind), kind)));
        }
    ...
    public override bool IsMatch(DateTime datetime)
    {
        ...
        var evalValue = Kind switch
        {
            ...
            CrontabFieldKind.DayOfWeek => ToCronDayOfWeek(datetime.DayOfWeek),
    ...
    public int? Next(int currentValue)
    {
        ...
        {
            throw new CronExpressionException("Cannot call Next for Day, Month or DayOfWeek types.");
        }
    ...
    public int First()
    {
        ...
        if (Kind == CrontabFieldKind.Day
            ...
            || Kind == CrontabFieldKind.DayOfWeek)
        {
            throw new CronExpressionException("Cannot call First for Day, Month or DayOfWeek types.");
        }
