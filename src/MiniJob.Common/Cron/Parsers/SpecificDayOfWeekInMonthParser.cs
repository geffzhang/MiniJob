...
/// </remarks>
internal sealed class SpecificDayOfWeekInMonthParser : CronParserBase
{
    /// <summary>
    /// 构造函数
    /// </summary>
    ...
    public SpecificDayOfWeekInMonthParser(int dayOfWeek, int weekNumber, CrontabFieldKind kind)
    {
        ...
        {
            throw new CronExpressionException(string.Format("Week number = {0} is out of bounds.", weekNumber));
        }

        // 验证 L 字符是否在 DayOfWeek 字段域中使用
        if (kind != CrontabFieldKind.DayOfWeek)
        {
            throw new CronExpressionException(string.Format("The <{0}#{1}> parser can only be used in the Day of Week field.", dayOfWeek, weekNumber));
        }
