...
/// </remarks>
internal sealed class NearestWeekdayParser : CronParserBase
{
    /// <summary>
    /// 构造函数
    /// </summary>
    ...
    /// <exception cref="CronExpressionException">Cron 字段种类</exception>
    public NearestWeekdayParser(int specificValue, CrontabFieldKind kind)
    {
        // 验证 {0}W 字符是否在 Day 字段域中使用
        if (kind != CrontabFieldKind.Day)
        {
            throw new CronExpressionException(string.Format("The <{0}W> parser can only be used in the Day field.", specificValue));
        }
        ...
        {
            throw new CronExpressionException(string.Format("The <{0}W> is out of bounds for the Day field.", specificValue));
        }
